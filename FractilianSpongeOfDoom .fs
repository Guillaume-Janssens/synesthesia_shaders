...
void main() 
{
    ...
    vec3 no = norm(p);
    vec3 col = vec3(R*p.z,G+p.y+p.x,B)+0.525;
    vec3 bounce = ray(p+0.04*d,d);
    col = mix(col,vec3(0.0),dot(no, normalize(light.xyz-p*0.5)));
    if (length(bounce-light.xyz) > light.w+0.2) col *= 0.2;
